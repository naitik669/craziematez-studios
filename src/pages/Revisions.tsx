...
export default function Revisions() {
  ...
  const { openRevision } = useEnhancedModals(); // ✅ moved inside component
    ...
    <div className="p-6 space-y-5">
      <div>
        ...
        <p className="text-neutral-500 text-sm mt-0.5">
          {data.stats.total_revisions} total revision requests
        </p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        {(["high", "medium", "low"] as const).map((p) => {
          const count = data.revisions.filter(
            (r) => r.priority === p
          ).length;
          ...
          return (
            ...
            >
              ...
              <p className="text-[10px] uppercase tracking-widest text-neutral-600 mt-1 capitalize">
                {p} Priority
              </p>
            </div>
          );
        ...
        <div className="space-y-3">
          {sorted.map((r, i) => {
            ...
            return (
              <motion.div
                key={r.id}
                onClick={() => openRevision(r)} // ✅ fixed placement
                style={{ cursor: "pointer" }}   // ✅ fixed placement
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <div>
                      <span className="font-mono-jet text-sm font-semibold text-white">
                        {r.scene}
                      </span>
                      {r.member_name && (
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className="text-[10px] font-semibold px-2 py-0.5 rounded-full capitalize font-mono-jet"
                        style={{
                          color: pc,
                          background: `${pc}15`,
                      ...
                      <span className="text-[11px] text-neutral-600 font-mono-jet">
                        {r.date?.slice(0, 10)}
                      </span>
                    </div>
                  </div>
