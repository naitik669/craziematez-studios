...
export default function Meetings() {
  ...
  const { openMeeting } = useEnhancedModals(); // ✅ added
  ...
  const upcoming = data.meetings
    .filter(m => !m.cancelled && new Date(m.meeting_datetime) >= now)
    .sort(
      (a, b) =>
  ...
  function MeetingCard({
    ...
    return (
      <motion.div
        onClick={() => openMeeting(m)} // ✅ added here
        ...
        <div className="flex items-start gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
              dim ? "bg-white/[0.04]" : "bg-orange-500/10"
            }`}
          >
            <Calendar
              size={16}
              className={dim ? "text-neutral-600" : "text-orange-400"}
            />
          </div>
          ...
          <div className="flex-1">
            <p
              className={`font-semibold text-sm ${
                dim ? "text-neutral-600" : "text-white"
              } leading-tight`}
            >
              {m.title}
            </p>
            ...
            <div className="flex items-center gap-3 mt-1.5 flex-wrap">
              ...
              {soon && (
                <span className="text-[10px] text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded-full font-semibold">
                  SOON
                </span>
              )}
            </div>
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-neutral-500 text-sm mt-0.5">
          {data.stats.upcoming_meetings} upcoming
        </p>
      </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {past.slice(0, 5).map((m, i) => (
              <MeetingCard key={m.id} m={m} i={i} dim />
            ))}
          </div>
        </div>
      )}
